...
class Fusion360PaletteBase:
    ...
    def on_palette_create(self, command: adsk.core.Command, args):
        global handlers
        ...
        try:
            ...
            onExecute = ShowPaletteCommandExecuteHandler(self)
...
# class ShowPaletteCommandCreatedHandler:
#     def __init__(self):
#         super().__init__()
#
#     def notify(self, args):
...
class ShowPaletteCommandExecuteHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self):
        try:
            ...
            palette = _ui.palettes.itemById(self.cmd_id)
            if not palette:

                palette = _ui.palettes.add(self.cmd_id, self.cmd_name, 'Tutorials.html', True, True, True, 400, 250)
                cmd = args.command
                cmd.helpFile = 'help.html'
                ...
                onHTMLEvent = MyHTMLEventHandler(self)
                palette.incomingFromHTML.add(onHTMLEvent)
                handlers.append(onHTMLEvent)

                # Add handler to CloseEvent of the palette.
                onClosed = MyCloseEventHandler(self)
...
class MyCloseEventHandler(adsk.core.UserInterfaceGeneralEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            if palette:
                palette.deleteMe()
            panel = _ui.allToolbarPanels.itemById(self.toolbar_panel_id)
            cmd = panel.controls.itemById(self.cmd_id)
            if cmd:
                cmd.deleteMe()
            cmdDef = _ui.commandDefinitions.itemById(self.cmd_id)
            if cmdDef:
                cmdDef.deleteMe()
